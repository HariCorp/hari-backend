...
import { Controller, Get, Post, Body, Param, Put, Delete, Query, UseGuards, Logger, NotFoundException, ForbiddenException } from '@nestjs/common';
import { ProductService } from './product.service';
import { CreateProductDto, UpdateProductDto, FilterProductDto } from '@app/common';
...
export class ProductController {
  ...
  async findAll(@Query() filterDto: FilterProductDto) {
    this.logger.log(`Getting all products with filters: ${JSON.stringify(filterDto)}`);
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @RBAC('read', 'product')
  async findByCategory(@Param('categoryId') categoryId: string, @Query() filterDto: FilterProductDto) {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @RBAC('read', 'product')
  async findByUser(@Param('userId') userId: string, @Query() filterDto: FilterProductDto) {
