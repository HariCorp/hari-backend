...
export class ProductServiceController {
  ...
  async findAll(command){
    console.log('[' + new Date().toLocaleTimeString() + '] 🔍 [hari-backend/apps/product-service/src/product-service.controller.ts:33] - ' + command)
    try {
      ...
      console.log('[' + new Date().toLocaleTimeString() + '] 🔍 [hari-backend/apps/product-service/src/product-service.controller.ts:42] - ' + error)
      return {
        status: 'error',
        error: {
