...
import { UploadedFile, UploadedFileDocument, FileType } from '../schemas/uploaded-file.schema';
...
export class UploadService {
   ...
   */
  async uploadFile(uploadFileDto: UploadFileDto): Promise<UploadedFileDocument> {
    ...
    try {
      ...
      this.logger.log(`Đã upload file lên Cloudinary: ${uploadResult.public_id}`);
      
      // Lưu thông tin file vào database
      const fileMetadata = this.extractMetadata(uploadResult, uploadFileDto.fileType);
      ...
      this.logger.log(`Đã lưu thông tin file vào database: ${uploadedFile._id}`);
      return uploadedFile;
    } catch (error) {
   ...
   * @returns Kết quả xóa
   */
  async deleteFile(publicId: string): Promise<{ deleted: boolean; publicId: string }> {
  ...
  async findAllFiles(filter: { 
    ...
    try {
      ...
      const files = await this.uploadedFileModel.find(query).sort({ createdAt: -1 }).exec();
   ...
   */
  private mapResourceType(fileType: FileType): 'image' | 'video' | 'raw' | 'auto' {
    switch (fileType) {
      case FileType.IMAGE:
        return 'image';
      case FileType.VIDEO:
